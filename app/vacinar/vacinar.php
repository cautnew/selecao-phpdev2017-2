...
$html .= $form->addInput('text', 'p__vac_var_nome', false, array('placeholder' => 'Nome do animal', 'class' => 'sepV_b m-wrap small'), false, false, false);
...
<script>
    ...
    function filtrar(page) {
        vacinarLoad('', '', '', $('#filter').serializeObject(), page);
        return false;
    }
    ...
    $(function() {
        ...
        $(document).on('click', '.l__btn_editar, tr.linhaRegistro td:not([class~="acoes"])', function() {
            ...
            loadForm(URL_API + 'animalVacina/' + ani_int_codigo, function(json) {
        ...
        $(document).on('click', '.l__btn_excluir', function() {
            ...
            $.gDisplay.showYN("Quer realmente deletar o item selecionado?", function() {
                $.gAjax.exec('DELETE', URL_API + 'animalVacina/' + ani_int_codigo, false, false, function(json) {
                    if (json.status) {
                        filtrar();
                    }
                });
